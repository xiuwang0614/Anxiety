...
%% Original factor
inputDir_best = fullfile(CODE_DIR,'data_release','files_polarLDA','model_K4');

in_dir = fullfile('I:\lda\bootstrap\output');
...
Mean_best_net(4,:) = CBIG_ASDf_Plot400Schaefer19Subcor17Networks_NetworksOnly(Mean_best(4,:), scalelim_blk, [], is_sum);
close all;
...
f4_net = zeros(N, M_blk);

% Loop over all bootstrapped samples
for i = 1:N
    input_dir = fullfile(in_dir, ['resampled_' num2str(i)],'estimate');
    ...
    f4_net(i,:) = CBIG_ASDf_Plot400Schaefer19Subcor17Networks_NetworksOnly(Mean(4,:), scalelim_blk, [], is_sum);
    close all;
end
out_dir = 'I:\lda\0611\bootstrapZscore';
save(fullfile(out_dir,'bootstrappedFactors.mat'), 'f1_all', 'f2_all', 'f3_all','f4_all');
save(fullfile(out_dir,'bootstrappedFactorsByNetworks.mat'), 'f1_net', 'f2_net', 'f3_net','f4_net');
