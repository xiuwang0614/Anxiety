...
% inputDir = fullfile(inputDir, ['k4'], ['r26']);
% outputDir = fullfile(outputDir, ['k4'], ['r26']);
num_ROIs = 416;
...
%for factor_idx = 1:4
    ...
    %end
    [rcorr_mat] = CBIG_ASDf_Plot400Schaefer19Subcor17Networks_419by419Input(mean_corrmat, ...
        scalelim, fullfile(outputDir, ['mean' num2str(factor_idx)]));
     save(fullfile(outputDir,['rmean' num2str(factor_idx) '.mat']), 'rcorr_mat');
     csvwrite(fullfile(outputDir,['rmean' num2str(factor_idx) '.csv']),rcorr_mat);
end
