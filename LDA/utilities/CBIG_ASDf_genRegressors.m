...
%因为没有id_motion
if nargin == 1 || isempty(sub_id)
    [id_sites, ~, id_age, id_sex] = CBIG_ASDf_getSubData(sub_info_file);
else
    [id_sites, ~, id_age, id_sex] = CBIG_ASDf_getSubData(sub_info_file, sub_id);
end

id_sites = cellfun(@num2str, id_sites, 'UniformOutput', false);
id = id_sites(:,1);
sites = id_sites(:,2);
