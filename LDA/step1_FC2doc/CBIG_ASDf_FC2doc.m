...
% Instead, it's identifying elements in dx_info that match the value stored in cohort_label(1)
% Y_CN = corrArr(indCN,:);
% mean_CN = mean(Y_CN,1);
% save([output_dir output_name '_mean_CN.mat'], 'mean_CN');
% X_CN = [ones(size(Y_CN,1),1) regressors(indCN,:)];

indCN = dx_info==cohort_label(2);
Y_CN = corrArr(indCN,:);
mean_CN = mean(Y_CN,1);
save([output_dir output_name '_mean_CN.mat'], 'mean_CN');
X_CN = [ones(size(Y_CN,1),1) regressors(indCN,:)];
...
if find(isnan(discretized_Z))
    ...
    discretized_Z(isnan(discretized_Z)) = 0;
end
